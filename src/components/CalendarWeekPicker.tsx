...
interface Props {
  ...
  initialMonth: number;
  initialWeek: number;
  onConfirm: (year: number, month: number, week: number) => void;
  onClose: () => void;
}

export default function CalendarWeekPicker({ initialYear, initialMonth, initialWeek, onConfirm, onClose }: Props) {
  ...
  const weekItems = Array.from({ length: totalWeeks }, (_, i) => i + 1);

  const handleYearChange = useCallback((y: number) => {
    setTempYear(y);
    setTempWeek(prev => Math.min(prev, weeksInMonth(y, tempMonth)));
  }, [tempMonth]);

  const handleMonthChange = useCallback((m: number) => {
    setTempMonth(m);
    setTempWeek(prev => Math.min(prev, weeksInMonth(tempYear, m)));
  }, [tempYear]);
